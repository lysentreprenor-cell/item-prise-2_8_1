...
function SummarySection({ title, onEdit, children }: { title: string; step: number; onEdit: () => void; children: React.ReactNode }) {
  return (
...
export default function Step6Summary({ data, totalAmount, goToStep, onCreateContract }: Props) {
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.header}><Text style={styles.stepNum}>Krok 6</Text><Text style={styles.stepTitle}>Podsumowanie</Text><Text style={styles.stepDesc}>Sprawdź wszystkie dane przed utworzeniem umowy.</Text></View>
      ...
      <SummarySection title="Zakres prac" step={2} onEdit={() => goToStep(2)}>
        <SummaryRow label="Pomieszczenia" value={data.rooms.length > 0 ? data.rooms.map(r => r.name).join(', ') : '—'} warning={data.rooms.length === 0} />
        <SummaryRow label="Łączna pow." value={data.rooms.reduce((s, r) => s + r.area, 0) > 0 ? `${data.rooms.reduce((s, r) => s + r.area, 0)} m²` : '—'} />
        <SummaryRow label="Instalacje" value={[data.hasElectrical && 'Elektryka', data.hasPlumbing && 'Hydraulika', data.hasMaterials && 'Materiały'].filter(Boolean).join(', ') || 'Brak'} />
      </SummarySection>
      <SummarySection title="Wycena" step={3} onEdit={() => goToStep(3)}>
        {data.pricingMethod === 'per_m2' && <><SummaryRow label="Powierzchnia" value={`${data.totalArea} m²`} /><SummaryRow label="Cena za m²" value={formatCurrency(data.pricePerM2)} /></>}
        {data.pricingMethod === 'ryczalt' && <SummaryRow label="Ryczałt" value={formatCurrency(data.lumpSumPrice)} />}
        {data.pricingMethod === 'godzinowy' && <><SummaryRow label="Stawka/h" value={formatCurrency(data.hourlyRate)} /><SummaryRow label="Godziny" value={`${data.estimatedHours}h`} /></>}
      ...
      <SummarySection title="Płatność" step={4} onEdit={() => goToStep(4)}>
        ...
        {bd.total > 0 && <><SummaryRow label="Depozyt" value={formatCurrency(bd.deposit)} />{bd.afterCompletion > 0 && <SummaryRow label="Po wykonaniu" value={formatCurrency(bd.afterCompletion)} />}</> }
      </SummarySection>
      <SummarySection title="Warunki" step={5} onEdit={() => goToStep(5)}>
        ...
        <SummaryRow label="Dowody" value={data.requiredProofs.length > 0 ? `${data.requiredProofs.length} wymaganych` : 'Brak'} />
        <SummaryRow label="Termin poprawek" value={data.correctionDays > 0 ? `${data.correctionDays} dni` : 'Nie określono'} warning={data.correctionDays === 0} />
        <SummaryRow label="Wypłata depozytu" value={data.paymentDeadlineDays > 0 ? `${data.paymentDeadlineDays} dni od odbioru` : 'Nie określono'} />
      </SummarySection>
      ...
      <TouchableOpacity style={[styles.createBtn, warnings.length > 0 && styles.createBtnWarning]} onPress={onCreateContract} activeOpacity={0.85}>
        <Text style={styles.createBtnText}>{warnings.length > 0 ? '⚠️ Utwórz mimo braków' : '✓ Utwórz profesjonalną umowę'}</Text>
      </TouchableOpacity>
